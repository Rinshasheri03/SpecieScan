...
def read_dataset():
    data_list = []
    label_list = []
    my_list = os.listdir(r'C:\Users\user\Pictures\dataset')
    for pos,pa in enumerate(my_list):

        print(pa,"==================")
        for root, dirs, files in os.walk(r'C:\Users\user\Pictures\dataset\\' + pa):

         for f in files:
            file_path = os.path.join(r'C:\Users\user\Pictures\dataset\\'+pa, f)
            print(file_path)
...
if not os.path.exists("modelgen.h5"):

    model.fit_generator(train_generator, steps_per_epoch=batch_size, epochs=epochs)
    model.save("modelgen.h5")  # train for randomly selected one
else:

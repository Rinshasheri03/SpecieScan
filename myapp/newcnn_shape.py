...
def read_dataset():
    data_list = []
    label_list = []
    my_list = os.listdir(r'C:\Users\user\Pictures\Wings')
    for pos,pa in enumerate(my_list):

        print(pa,"==================")
        for root, dirs, files in os.walk(r'C:\Users\user\Pictures\Wings\\' + pa):

         for f in files:
            file_path = os.path.join(r'C:\Users\user\Pictures\Wings\\'+pa, f)
            print(file_path)
...
if not os.path.exists("modelshape.h5"):

    model.fit_generator(train_generator, steps_per_epoch=batch_size, epochs=epochs)
    model.save("modelshape.h5")  # train for randomly selected one
else:
